...
let currentLocation; let locationAccuracy; let userPosition; let selectedDestination; let routeDestination; let routeData; let routeReady = false; let navigationActive = false; let locationWatchId;
...
function attachSuggestions(input, container, onSelect) {
	...
	input.addEventListener('input', () => {
		...
		if (input.id === 'destination-input') { selectedDestination = null; routeReady = false; routeDestination = null; navigationActive = false; document.getElementById('start-route').innerHTML = 'Route berekenen <span>→</span>'; }
...
document.getElementById('preview-route').addEventListener('click', () => { if (routeLine) { map.fitBounds(routeLine.getBounds(), { padding: [80, 80], animate: true }); routePanel.classList.add('is-open'); } });
...
searchInput.addEventListener('input', () => clearSearch.classList.toggle('is-visible', searchInput.value.length > 0)); clearSearch.addEventListener('click', () => { searchInput.value = ''; clearSearch.classList.remove('is-visible'); searchInput.focus(); });
